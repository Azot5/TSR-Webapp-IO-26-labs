...
# Database setup with paths in Data bases folder
...
user_engine = create_engine(config.USER_DATABASE_URL, connect_args={"check_same_thread": False})
admin_engine = create_engine(config.ADMIN_DATABASE_URL, connect_args={"check_same_thread": False})
queue_engine = create_engine(config.QUEUE_DATABASE_URL, connect_args={"check_same_thread": False})
vaccine_engine = create_engine(config.VACCINE_DATABASE_URL, connect_args={"check_same_thread": False})
...
def log_action(db_type: str, username: str, action: str, response: str):
    ...
    if db_type == "user":
        ...
        # Default to admin log if type not specified
        admin_logger.info(log_message)
...
async def maintenance_middleware(request: Request, call_next):
    ...
    # Quarantine mode affects all endpoints (including admin)
    if SERVER_QUARANTINE_MODE and request.url.path != "/maintenance":
        return RedirectResponse(url="/maintenance")
    
    # Maintenance mode only affects non-admin endpoints
